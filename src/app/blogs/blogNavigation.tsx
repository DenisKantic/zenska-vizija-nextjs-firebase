...
export default function BlogNavigation({ blogId, setBlog, blog }: any) {
  ...
  return (
    <div className='h-1/3'>
      <h3 className='text-slate-700 text-2xl font-semibold mb-8 px-6'>Istaknuti članci</h3>
      {
        blogs.slice(0, 5).map((listItem) => (
          <div key={listItem.id} onClick={() => settingBlog(listItem.id)} className={`${listItem.title === blog.title ? 'bg-[#F93EDF] text-white' : ''} hover:bg-orange-600 hover:text-white cursor-pointer px-6 py-4 rounded-xl mb-2`}>
            <p>{listItem.title}</p>
          </div>
        ))
      }
